...
class MiniAppSessionStoreTestCase(unittest.TestCase):
    ...
    def test_active_session_is_recoverable_only_by_owner_and_exact_init_data(self) -> None:
        ...
        # Contribution PUSH now travels over Telegram sendData, so a
        # recoverable session carries no HTTP capability state at all.
        self.assertFalse(hasattr(session, "contribution_capability_token"))
        self.assertFalse(hasattr(session, "contribution_capability_issued"))
