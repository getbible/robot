...
_SYNC_ID_RE = re.compile(r"[A-Za-z0-9._:-]{1,128}\Z")
...
class MiniAppApi:
    ...
    async def _handle(self, request: MiniAppHttpRequest) -> MiniAppHttpResponse:
        ...
        try:
            ...
            if parts.path == f"{self._api_prefix}/contributions/receipt":
                if method != "GET":
                    return self._method_not_allowed("GET, OPTIONS")
                return self._contribution_receipt(session, parts.query)
    ...
    def _allowed_methods(self, path: str) -> tuple[str, ...] | None:
        ...
        exact: dict[str, tuple[str, ...]] = {
            ...
            f"{self._api_prefix}/contributions/receipt": ("GET",),
    ...
    def _contribution_receipt(
        ...
        """Confirm one pushed synchronization so the browser can settle its outbox."""
        values = _query(query)
        if set(values) != {"sync_id"}:
            raise MiniAppApiInputError("Receipt request requires exactly sync_id.")
        sync_id = values["sync_id"]
        if _SYNC_ID_RE.fullmatch(sync_id) is None:
            raise MiniAppApiInputError("sync_id is invalid.")
        store = self._contributions
        if store is None:
            return self._contributions_unavailable_response()
        try:
            receipt = store.sync_receipt(session.user_id, sync_id)
        except (ContributionError, OSError, sqlite3.Error, RuntimeError):
            LOGGER.warning(
                "A contribution receipt is temporarily unavailable",
                exc_info=True,
            )
            return self._contributions_unavailable_response()
        if receipt is None:
            return self._response(200, {"found": False, "receipt": None})
        event_ids = cast(dict[str, int], receipt["event_ids"])
        return self._response(
            200,
            {
                "found": True,
                "receipt": {
                    ...
                    # contributor-scoped receipts avoid exposing global
                    # submission volume through AUTOINCREMENT gaps.
                    "event_ids": {
                        client_event_id: _contribution_receipt_id(
                            session.user_id,
                            client_event_id,
                        )
                        for client_event_id in event_ids
                    },
                },
            },
        )
