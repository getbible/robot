...
async def _allow_command(
    ...
    """Apply both user and chat budgets before any command response or lookup."""
    ...
    try:
        await limiter.acquire(user_id=user_id, chat_id=update.effective_chat.id)
    except RobotRateLimited as error:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Too many requests. Please try again in about {error.retry_after} seconds.",
        )
        return False
    return True
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_chat is None:
        return
    settings, _, limiter = _components(context)
    try:
