...
Together they expose the application's concurrency, session, cache, timeout,
rate, abuse, memory, CPU, PID, tmpfs, and log-retention controls without
...
`docker-restart` deliberately uses Compose to recreate the container. A plain
`docker restart` does not reload changed environment variables or Compose
resource settings. Pass `--env-file`, `--secure`, or `--multi` to the setup
commands when using those matching deployment variants. `docker-config
--no-restart` validates and saves a change without applying it yet.
