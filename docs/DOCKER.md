...
expose the image release plus the application's concurrency, session,
...
posting, or Mini App interface. The Telegram-native search worker has an
independent bounded executor and circuit for its requests to the public
Search API. A slow search therefore does not consume the direct Scripture
lookup workers or block ordinary Telegram update handling. Mini App searches
go from the browser to the Search API and never enter the container.

The supplied production defaults use:

- sixteen concurrent Telegram updates;
- eight direct Scripture/catalog workers;
- four Search API request workers for the Telegram-native `/search`;
...
the container cannot consume the host in an endless restart loop.
...
fractional `MINI_APP_NAVIGATION_RATE_COST`; session exchange, posting, and
bookmark backup retain full request cost, while Scripture reads and search
never reach the container. Normal browsing therefore does not exhaust the
same budget intended to stop expensive request floods.
...
The robot no longer holds a translation corpus or search index. A search is
an outbound HTTPS request to the public Search API — from the browser for the
Mini App, from the container only for the Telegram-native `/search` — so the
bot's resident size does not grow with the translations searched and nothing
is downloaded or built at startup. Memory is spent on the Python runtime,
Tornado, and the bounded session, interaction, and rate-limit state. The
profile below predates this change and is retained as a conservative
ceiling; measure the deployed image before lowering it. See
[Search](SEARCH.md).
...
- persistent preference and private contribution state;
